...
const query  =require('../../libs/async-db');
  ...
  router.get('/',async function(ctx){
     let sql = `SELECT id,title,content,creat_time,modules FROM about WHERE modules='design'`;
      let data = await query.selectAllData(sql);
      await ctx.render('admin/design',{nav_now:'design',username:ctx.session.username,about:data[0]});
