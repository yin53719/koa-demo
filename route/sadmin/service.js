...
const query  =require('../../libs/async-db');
  ...
  router.get('/',async function(ctx){
     let sql = `SELECT id,title,content,creat_time,modules FROM about WHERE modules='service'`;
      let data = await query.selectAllData(sql);
      await ctx.render('admin/service',{nav_now:'service',username:ctx.session.username,about:data[0]});
